...
extract_prompt = PromptTemplate(
    ...
    template="""История предыдущего диалога: {history}

    Новое сообщение водителя: {message}

    Извлеки структурированную информацию. Учитывай контекст: если водитель использует местоимения или ссылается на предыдущие сообщения (например, "это", "оно", "та же проблема") — определи, о чём речь, по истории диалога.
